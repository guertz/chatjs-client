...
extern char _binary_assets_picodom_js_start[]; /**< Riferimento inizio file picodom.js framework js */

extern char _binary_assets_picostyle_js_start[]; /**< Riferimento inizio file picostyle.js framewok stili js */
...
namespace WebUI {
    ...
    void JsHCallback(Webview *w, const char *args) {
        ...
        json jArgs = json::parse(args); /** oggetto json deserializzato */

        Methods::iterator fn_itr = callbacks.find(jArgs["fn"]); /** Iteratore dell'oggetto map specializzato
                                                                    dal tipo di dato Methods che rappresenta
                                                                    l'oggetto contenente il puntatore a funzione */
        
        if (fn_itr != callbacks.end())
            fn_itr->second(safestr::duplicate(args)); // Se iteratore valido invoco la funzione duplicato il buffer
    ...
    void Inject() {
        ...
        const char* const picodom = reinterpret_cast<const char* const>(&_binary_assets_picodom_js_start);
        const char* const picostyle = reinterpret_cast<const char* const>(&_binary_assets_picostyle_js_start);
        ...
        webview_dispatch(&webview,Dispatch, js::prepare(picodom));
        webview_dispatch(&webview,Dispatch, js::prepare(picostyle));
    ...
    void Log(const char* argl) {
        ...
        log_csl(
                log_data["params"]["attr"].get<string>().c_str(),
                log_data["params"]["log_msg"].get<string>().c_str()
            );
